[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/b842RB8g)
# Tugas Besar 1 IF2010 Pemrograman Berorientasi Objek

## Cara Compile dan Run
1. ke `root` directory jalanin command `make`
2. ke `bin` directory jalanin command `./game`

## Alur Program
1. Input file config yang diinginkan
2. Memilih mode apakah melanjutkan permainan lama (muat savegame = y) atau memulai permainan baru
3. Mendaftarkan pemain satu per satu
4. Permainan dimulai! yippie :)

## Kontributor
| NIM | Nama |
| --- | --- |
| 13524008 | Agatha Tatianingseto |
| 13524030 | Irvin Tandiarrang Sumual |
| 13524074 | Bernhard Aprillio Pramana |
| 13524096 | Moreno Syawali Ganda Sugita |
| 13524110 | Jennifer Khang |

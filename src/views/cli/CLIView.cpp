...
int CLIView::boardSize() const {
    return board_->getSize();
}

void CLIView::showMessage(std::string message) {
    std::cout << message;
}

void CLIView::setBoardContext(Board* board) {
    board_ = board;
}

void CLIView::cetakPapan(Player& pl, int currentTurn) {
    if (board_ == nullptr) {
...
void CLIView::cetakProperti(Player* player) {
    if (board_ == nullptr) {
        std::cout << "Board belum tersedia.\n";
        return;
    }
    ...
    auto padRight = [](std::string s, size_t width) {
    ...
    for (const std::string& kat : urutanKategori) {
        ...
        if (groupedProps.find(kat) != groupedProps.end() && !groupedProps[kat].empty()) {
            ...
            for (PropertyTile* prop : groupedProps[kat]) {
                ...
                if (prop->propertyStatusToText() == "MORTGAGED") {
